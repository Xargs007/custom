...
c:EnableReviveLimit()
	...
	e1:SetValue(c911000150.splimit)
	c:RegisterEffect(e1)
	--special summon rule
	...
	e2:SetCondition(c911000150.sprcon)
	e2:SetOperation(c911000150.sprop)
	c:RegisterEffect(e2)
	--effect
...
c911000150.material={511000151,511000152,511000153,511000154,40640057}
function c911000150.splimit(e,se,sp,st)
	return e:GetHandler():GetLocation()~=LOCATION_EXTRA
end
function c911000150.sprfilter(c,code)
	return c:IsCode(code)
end
function c911000150.sprcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.IsExistingMatchingCard(c911000150.sprfilter,tp,LOCATION_ONFIELD,0,1,nil,511000151)
		and Duel.IsExistingMatchingCard(c911000150.sprfilter,tp,LOCATION_ONFIELD,0,1,nil,511000152)
		and Duel.IsExistingMatchingCard(c911000150.sprfilter,tp,LOCATION_ONFIELD,0,1,nil,511000153)
		and Duel.IsExistingMatchingCard(c911000150.sprfilter,tp,LOCATION_ONFIELD,0,1,nil,511000154)
		and Duel.IsExistingMatchingCard(c911000150.sprfilter,tp,LOCATION_ONFIELD,0,1,nil,40640057)		
end
function c911000150.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g1=Duel.SelectMatchingCard(tp,c911000150.sprfilter,tp,LOCATION_ONFIELD,0,1,1,nil,511000151)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g2=Duel.SelectMatchingCard(tp,c911000150.sprfilter,tp,LOCATION_ONFIELD,0,1,1,nil,511000152)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g3=Duel.SelectMatchingCard(tp,c911000150.sprfilter,tp,LOCATION_ONFIELD,0,1,1,nil,511000153)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
...
function c911000150.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsAbleToExtraAsCost() end
	Duel.SendtoDeck(c,nil,0,REASON_COST)
end
function c911000150.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,4,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,4,0,0)
end
function c911000150.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsPlayerCanSpecialSummonMonster(tp,511000151,0,0x4011,300,200,1,RACE_FIEND,ATTRIBUTE_DARK) then
		...
		e1:SetCondition(c911000150.ncondition)
		e1:SetOperation(c911000150.noperation)
		e1:SetCountLimit(1)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		e4:SetCountLimit(1)
		e4:SetTarget(c911000150.target)
		e4:SetOperation(c911000150.operation)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		e3:SetTarget(c911000150.sptg2)
		e3:SetOperation(c911000150.spop2)
...
function c911000150.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c911000150.operation(e,tp,eg,ep,ev,re,r,rp)
...
function c911000150.ncondition(e,tp,eg,ep,ev,re,r,rp)
...
function c911000150.noperation(e,tp,eg,ep,ev,re,r,rp)
	Duel.NegateAttack()
end
...
function c911000150.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c911000150.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE)
end
function c911000150.spop2(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)==0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c911000150.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
	if tc then
